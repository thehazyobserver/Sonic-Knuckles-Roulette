# Note from Lunchtime:
The deployment scripts and tests are in `./src/backend` directory. The original fork was a hardhat project so for simplicity I retained it as such.  

Don't forget to set your deployer key in `.secret`!

You will see that after a deployment, in `./src/backend/contractsData` there are `-address.json` files where the deployed address is saved.  
Original dev used this system to have the contracts imported into react and should contain the current most recent deployment addresses on Fantom.
...
```shell
git clone git@github.com:0xlunch/roulette
```
