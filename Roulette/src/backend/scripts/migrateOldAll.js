...
async function main() {
    ...
    const withdraw = await roulette.withdrawTokens(rouletteBalance);
    ...
    if(myPostBalance.lt(rouletteBalance)) {
        console.error("Some reason balance is low. Check chain and txs. Quitting.");
        return;
    }

    const transfer = await Knuckles.transfer(treasuryAddress, rouletteBalance);
    const transferReceipt = await transfer.wait();

    console.log("Transfer of 100% complete, tx:", transferReceipt.transactionHash);
}
